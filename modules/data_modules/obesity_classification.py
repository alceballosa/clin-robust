...
class ObesityClassification(AbstractDataModule):
    ...
    def __init__(
        ...
        pred_set_path = os.path.join(root_path, "n2c2", "obesity-classification-2008_b")
        self.pred_set = load_from_disk(pred_set_path)["test"].select_columns(
            ["text", "Obesity_intuitive"]
        )

        self.pred_set = self.pred_set.rename_column("text", "input")
        self.pred_set = self.pred_set.rename_column("Obesity_intuitive", "output")
