...
# TODO: switch back to using Fabric here
class LogitsMetric:
    ...
    def __call__(self, classes, gold_classes):
        ...
        length = torch.tensor(len(classes), dtype=torch.int32, device=self.fabric)
...
class OutputMetric:
    ...
    def __call__(self, preds, golds):
        correct = torch.tensor(0, dtype=torch.int32, device=self.fabric)  # .device)
        length = torch.tensor(
            len(preds), dtype=torch.int32, device=self.fabric
        )  # .device)
